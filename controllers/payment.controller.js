...
const fs = require('fs');
const createTransactionReceipt = require("../utils/createTransactionRecepit.js")
...
const sendPayment = asyncHandler(async (req, res) => {
  ...
  // receiverPublicKey = receiverPublicKey?.replace(/ /g, '\n');


  // const pemFormatKey = toPemFormat(receiverPublicKey)
  // convert to base64 first or pem
  ...
  const receiverPublicKeyPem = Buffer.from(receiverPublicKey, 'base64').toString('utf-8');
...
const sendMails = asyncHandler(async (req, res) => {
  ...
  const user = req.user;
  // const { privateKey } = await Keys.findOne({ userId: user._id }).select('privateKey');
  const { privateKey } = await Keys.findOne({ userId: receiverId }).select('privateKey');

  const { encryptedData } = await Transaction.findOne().sort({ createdAt: -1 });
  ...
  const senderEmail = await User
    .findOne({ _id: user?._id.toString() })
    .select("email")

  const receiverEmail = await User
    .findOne({ _id: receiverId.toString() })
    .select("email")
  ...
  await createTransactionReceipt(details)
  ...
  <body style="margin: 0 !important; padding: 0 !important; background-color: #f4f7f6;">
    ...
    `, 
    attachments: [{
      filename: 'Transaction_Recepit.pdf',
      path: path.join(__dirname, '..', 'pdfs', 'Transaction_Recepit.pdf'),
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, { data: decrypted.toString() }, 'Email sent successful.'));
});

module.exports = {
  sendPayment, sendMails
};
